...
function processImg(file)
{
    ...
    console.log('DEBUG: handling image file ' + filehead + '_min.' + fileext + ' from ' + filesrc );

    return gulp.src(filesrc)
    .pipe( imagemin( { optimizationLevel : 5 } ) )
    .pipe(rename({basename: fileparts.name + '_min.' , extname:fileext }) )
...
{
    console.log('DEBUG: image processing started...');
    var stream = processImg(images);
    
    stream.on('end', function()
    {
        console.log('DEBUG: image processing completed');
        process.send('DEBUG: image processing completed');
        process.exit();
    });
